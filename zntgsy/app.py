...
def run_task():
    ...
    if is_mock is not None and bool(is_mock):
        tid = f'{algorithm_type}_{algorithm_name}_-1'
        return jsonify(Result.ok(tid).__dict__), 200
    filter_conditions = {}
    ...
    future = csys.executor.submit(algorithm_factory.algorithm_run,
    ...
    try:
        result_path, err_msg = future.result(timeout=csys.ALGORITHM_TIMEOUT)
        ...
        if err_msg is None:
            ...
            return jsonify(Result(code=201, msg="产品个数不足5个无法推荐,请重新选择数据。", data=None).__dict__), 200
    except concurrent.futures.TimeoutError:
        future.cancel()
        return jsonify(Result(code=202, msg=f'算法超时运行{csys.TIMEOUT}秒', data=None).__dict__), 200
