...
class EHGNNRunner:
    ...
    class IntegratedGradients:
        def __init__(self, model, steps=50):
            ...
            self.steps = steps
            # 若模型在 GPU 上，确保存放 device
            self.device = next(model.parameters()).device if any(p.requires_grad for p in model.parameters()) else torch.device("cpu")

        def compute_integrated_gradients(self, investor_ids, product_ids, investor_feats, product_feats):
            """计算投资者和产品特征的积分梯度"""
            # 生成基线输入
            baseline_inv = torch.zeros_like(investor_feats).to(self.device)
            baseline_pro = torch.zeros_like(product_feats).to(self.device)
            ...
            for alpha in tqdm(np.linspace(0, 1, self.steps + 1), desc="计算积分梯度"):
                # 插值输入
                current_inv = baseline_inv + alpha * (investor_feats.to(self.device) - baseline_inv)
                current_pro = baseline_pro + alpha * (product_feats.to(self.device) - baseline_pro)
    ...
    def train_model(self, metrics_to_calculate):
        ...
        investor_ids = torch.LongTensor(np.array(self.full_df['investor_id'].values)).to(self.device)
        product_ids = torch.LongTensor(np.array(self.full_df['product_id'].values)).to(self.device)
        investor_feats = torch.FloatTensor(np.array(self.full_df[self.investor_features].values)).to(self.device)
        product_feats = torch.FloatTensor(np.array(self.full_df[self.product_features].values)).to(self.device)
        labels = torch.FloatTensor(np.array(self.full_df['reward'].values)).to(self.device)

        # 训练循环（加入 mini-batch）
        batch_size = 1024
        n = investor_ids.size(0)
        for epoch in range(self.epochs):
            ...
            for i in range(0, n, batch_size):
                ...
                b_inv = investor_ids[idx]
                b_pro = product_ids[idx]
                b_invf = investor_feats[idx]
                b_prof = product_feats[idx]
                b_lab = labels[idx]
                optimizer.zero_grad()
                out = self.model(b_inv, b_pro, b_invf, b_prof).squeeze()
                loss = criterion(out, b_lab)
                loss.backward()
                optimizer.step()
                epoch_loss += loss.item() * b_inv.size(0)
        ...
        # 计算评估指标
        metrics = {}
        # 将 labels 从 GPU 拷回 CPU 再转 numpy，并展平为 1D
    ...
    def generate_results(self, target_investor_id):
        ...
        with torch.no_grad():
            investor_ids = torch.LongTensor(np.array([target_investor_id] * len(all_products))).to(self.device)
            product_ids = torch.LongTensor(all_products).to(self.device)
            investor_feat_mean = target_data[self.investor_features].mean().values
            investor_feats = torch.FloatTensor(np.array([investor_feat_mean] * len(all_products))).to(self.device)
            product_feats = torch.FloatTensor(self.full_df.groupby('product_id')[self.product_features].mean().loc[all_products].values).to(self.device)
            scores = self.model(investor_ids, product_ids, investor_feats, product_feats).squeeze().cpu().numpy()
        ...
        investor_ids = torch.LongTensor(target_data['investor_id'].values).to(self.device)
        product_ids = torch.LongTensor(target_data['product_id'].values).to(self.device)
        investor_feats = torch.FloatTensor(target_data[self.investor_features].values).to(self.device)
        product_feats = torch.FloatTensor(target_data[self.product_features].values).to(self.device)
