...
def algorithm_run(dataset_path: str,
    ...
    try:
        if algorithm_type == sys.ALGORITHM_TYPE_BASE:
            ...
            return _algorithm_run_llm_explain(dataset_path=dataset_path,
                                              user_id=user_id,
                                              algorithm_name=algorithm_name,
                                              filter_conditions=filter_conditions,
                                              custom_investor_features=custom_investor_features,
        ...
        return None, f'产品个数不足5个无法推荐,请重新选择数据。'
...
def _algorithm_run_new_investor(dataset_path: str,
    ...
    if algorithm_name == sys.ALGORITHM_NEW_INVESTOR_COLD_RUN:
        ...
        return runner.run_comparison(filter_conditions=filter_conditions,
                                     data_path=data_path)
    elif algorithm_name == sys.ALGORITHM_NEW_INVESTOR_COLD_RUN_MISS:
        runner = miss_lqd.AlgorithmComparison(user_id=user_id,
                                              custom_investor_features=custom_investor_features,
                                              custom_product_features=custom_product_features)
        data_path = sys.get_file_path(dataset_path)
        return runner.run_comparison(filter_conditions=filter_conditions,
                                     data_path=data_path)
    elif algorithm_name == sys.ALGORITHM_NEW_INVESTOR_COLD_RUN_COLD:
        runner = cold_lqd.ColdRecommender(user_id=user_id,
                                          investor_features=custom_investor_features,
                                          product_features=custom_product_features,
...
def _algorithm_run_llm_explain(dataset_path: str,
                               user_id: str,
                               algorithm_name: str,
                               filter_conditions=None,
                               custom_investor_features: list = None,
    ...
    if algorithm_name == sys.ALGORITHM_EXPLAIN_GCN:
        cmab_runner = llm_explain.LLMExplainRunner(
            investor_features=custom_investor_features,
            product_features=custom_product_features,
            user_id=user_id,
        )
        data_path = sys.get_file_path(dataset_path)
        return cmab_runner.run(filter_conditions=filter_conditions,
                               data_path=data_path)
    else:
        print(f'算法:{algorithm_name} 暂不支持')
        return None, f'算法:{algorithm_name} 暂不支持'
