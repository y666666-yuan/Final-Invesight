...
import numpy as np
from algorithm.llm_gnn import llm_explain_mock
...
class TaskTool:
    ...
    def _check_algorithm_type(self):
        ...
        if self.algorithm_type == sys.ALGORITHM_TYPE_BASE:
            ...
            if self.algorithm_name not in sys.ALGORITHM_NEW_INVESTOR_NAMES:
                raise Exception(f"系统不支持的算法{str(self.algorithm_type)}_{self.algorithm_name}")
        elif self.algorithm_type == sys.ALGORITHM_TYPE_EXPLAIN:
            if self.algorithm_name not in sys.ALGORITHM_EXPLAIN_NAMES:
                raise Exception(f"系统不支持的算法{str(self.algorithm_type)}_{self.algorithm_name}")
        else:
            raise Exception(f"系统不支持的算法{str(self.algorithm_type)}_{self.algorithm_name}")
    ...
    def get_processed_investors(self, page_size: int, page_num: int) -> (dict, str):
        ...
        :return:
        """
        if self.algorithm_type == sys.ALGORITHM_TYPE_EXPLAIN and self.algorithm_name == sys.ALGORITHM_EXPLAIN_GCN:
            ...
            # 按 userno 分组，并将 Projectno 收集为列表
            grouped = df.groupby("userno")["Projectno"].apply(list).reset_index()
            # 构造目标 JSON 结构
            result = [
                {
                    "recommend_products": row["Projectno"],
    ...
    def get_processed_investor_detail(self, user_no: int):
        ...
        if self.algorithm_type == sys.ALGORITHM_TYPE_EXPLAIN and self.algorithm_name == sys.ALGORITHM_EXPLAIN_GCN:
            recommend_file = sys.get_file_path(
                ...
                "user_recommendations.csv")
            recommend_df = pd.read_csv(recommend_file)
            recommend_df = recommend_df.loc[recommend_df['userno'] == user_no]
            item_list = recommend_df["Projectno"].tolist()
            ...
            for item_no in item_list:
                explain = {}
                explain['product_id'] = item_no
                item_df = base_df[base_df['PROJECTNO'] == item_no]
                item_df_first_row = item_df.iloc[0]
                item_info = {}
                item_info['total'] = str(int(item_df_first_row[14]))
                item_info['apr_percent'] = str(item_df_first_row[15])
                item_info['term'] = str(int(item_df_first_row[17]))
                item_info['level'] = str(int(item_df_first_row[18]))
                item_info['project_invest_count'] = str(int(item_df_first_row[16]))
                item_info['repayment'] = DataSetTool.query_cn_name("REPAYMENT", int(item_df_first_row[13]))
                explain["item_info"] = item_info
                explain['explain'] = llm_explain_mock.get_llm_explain(user_id=int(user_no), item_id=int(item_no))
    ...
    def get_algorithm_list(cls, algorithm_type):
        ...
        elif algorithm_type == sys.ALGORITHM_TYPE_EXPLAIN:
            return sys.ALGORITHM_EXPLAIN_PARAMS
    ...
    def get_model_metrics(self):
        ...
        if self.algorithm_type == sys.ALGORITHM_TYPE_BASE:
            ...
            for item in rows_as_dicts:
                if item['Algorithm'] == 'ehgnn':
                    item['Algorithm'] = '异构图神经网络'
                elif item['Algorithm'] == 'investor_cf':
                    item['Algorithm'] = '基于投资者的协同过滤'
                elif item['Algorithm'] == 'product_cf':
                    item['Algorithm'] = '基于产品的协同过滤'
                elif item['Algorithm'] == 'ncf':
                    item['Algorithm'] = '神经协同过滤'
...
class DataSetTool:
    ...
    def build_investor_product_network_fast(self):
        ...
        full_df = full_df[full_df['reward'] == 1]
    ...
    def _get_similar_edges(cls, similar_file_path, user_no_list, node_prefix):
        ...
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                id1 = ids[i]
                id2 = ids[j]
                sim = df.at[id1, str(id2)]
                e_id = node_prefix + str(id1) + node_prefix + str(id2)
                if (not e_id in edge_ids) and (id1 in user_no_list) and (id2 in user_no_list):
                    edges.append(
                        BaseNetworkEdge(id=e_id, edge_from=node_prefix + str(id1), edge_to=node_prefix + str(id2),
                                        label='相似', value=sim).__dict__
                    )
                    edge_ids.append(e_id)
        return edges

    @classmethod
    def _get_similar_edges_fast(cls, similar_file_path, user_no_list, node_prefix):
        """
        根据上三角相似矩阵csv文件 和 筛选出来的 id 构建相似网络中的边
        :param similar_file_path: 上三角相似矩阵csv文件地址
        :param user_no_list: 筛选出来的 id
