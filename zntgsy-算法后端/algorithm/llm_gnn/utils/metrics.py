import os
import torch, pickle
import numpy as np
import pandas as pd


class Metric(object):
    def __init__(self, output_path):
        self.metrics = ["recall", "ndcg", "precision", "mrr", "f1"]
        self.k = [5]
        self.output_path = output_path

    def recall(self, test_data, r, k):
        right_pred = r[:, :k].sum(1)
        recall_n = np.array([len(test_data[i]) for i in range(len(test_data))])
        recall = np.sum(right_pred / recall_n)
        return recall

    def precision(self, r, k):
        right_pred = r[:, :k].sum(1)
        precis_n = k
        precision = np.sum(right_pred) / precis_n
        return precision

    def mrr(self, r, k):
        pred_data = r[:, :k]
        scores = 1.0 / np.arange(1, k + 1)
        pred_data = pred_data * scores
        pred_data = pred_data.sum(1)
        return np.sum(pred_data)

    def ndcg(self, test_data, r, k):
        assert len(r) == len(test_data)
        pred_data = r[:, :k]

        test_matrix = np.zeros((len(pred_data), k))
        for i, items in enumerate(test_data):
            length = k if k <= len(items) else len(items)
            test_matrix[i, :length] = 1
        max_r = test_matrix
        idcg = np.sum(max_r * 1.0 / np.log2(np.arange(2, k + 2)), axis=1)
        dcg = pred_data * (1.0 / np.log2(np.arange(2, k + 2)))
        dcg = np.sum(dcg, axis=1)
        idcg[idcg == 0.0] = 1.0
        ndcg = dcg / idcg
        ndcg[np.isnan(ndcg)] = 0.0
        return np.sum(ndcg)

    def f1_score(self, precision, recall):
        if precision + recall == 0:
            return 0
        return 2 * (precision * recall) / (precision + recall)

    def get_label(self, test_data, pred_data):
        """
        Converts the ground truth data and predicted data into a binary format for evaluation.
        """
        r = []
        for i in range(len(test_data)):
            ground_true = test_data[i]
            predict_topk = pred_data[i]
            pred = list(map(lambda x: x in ground_true, predict_topk))
            pred = np.array(pred).astype("float")
            r.append(pred)
        return np.array(r).astype("float")

    def eval_batch(self, data, topks):
        sorted_items = data[0].numpy()
        ground_true = data[1]
        r = self.get_label(ground_true, sorted_items)

        result = {}
        for metric in self.metrics:
            result[metric] = []

        for k in topks:
            for metric in result:
                if metric == "recall":
                    result[metric].append(self.recall(ground_true, r, k))
                if metric == "ndcg":
                    result[metric].append(self.ndcg(ground_true, r, k))
                if metric == "precision":
                    result[metric].append(self.precision(r, k))
                if metric == "mrr":
                    result[metric].append(self.mrr(r, k))
                if metric == "f1":
                    precision = self.precision(r, k)
                    recall = self.recall(ground_true, r, k)
                    result[metric].append(self.f1_score(precision, recall))

        for metric in result:
            result[metric] = np.array(result[metric])

        return result

    def eval(self, model, dataloader, ground_true, base_data, base_datayuan):
        result = {}
        for metric in self.metrics:
            result[metric] = np.zeros(len(self.k))

        batch_ratings = []
        ground_truths = []
        test_user_count = 0
        test_user_num = len(dataloader.dataset.user_list)
        for _, tem in enumerate(dataloader):
            if not isinstance(tem, list):
                tem = [tem]
            test_user = tem[0].numpy().tolist()
            batch_data = list(map(lambda x: x.long(), tem))
            # predict result
            with torch.no_grad():
                batch_pred = model.full_predict(batch_data)  # jiang模型预测输出用户的k个推荐结果
            test_user_count += batch_pred.shape[0]
            # filter out history items
            # batch_pred = self._mask_history_pos(batch_pred, test_user, dataloader)
            _, batch_rate = torch.topk(batch_pred, k=max(self.k))
            batch_ratings.append(batch_rate.cpu())
            # ground truth
            ground_truth = []
            for user_idx in test_user:
                ground_truth.append(ground_true[user_idx])
            ground_truths.append(ground_truth)
        assert test_user_count == test_user_num

        # calculate metrics
        data_pair = zip(batch_ratings, ground_truths)
        eval_results = []
        for _data in data_pair:
            eval_results.append(self.eval_batch(_data, self.k))
        for batch_result in eval_results:
            for metric in self.metrics:
                result[metric] += batch_result[metric] / test_user_num

        # 保存推荐结果
        recommendations = {}
        for batch_idx, (batch_rate, _) in enumerate(zip(batch_ratings, ground_truths)):
            for user_idx, user_rec in zip(test_user, batch_rate):
                recommendations[user_idx] = user_rec.tolist()

        # 加载映射文件
        # base_data = pd.read_csv('./data/ppdxy/data_base.csv')
        # base_datayuan = pd.read_csv('./data/ppdxy/data_baseyuan.csv')

        # 创建映射字典
        user_map = dict(zip(base_data['user'], base_datayuan['user']))
        item_map = dict(zip(base_data['item'], base_datayuan['item']))

        # 转换recommendations中的编号
        converted_recommendations = []
        for user_idx, items in recommendations.items():
            original_user = user_map.get(user_idx, user_idx)
            for rank, item in enumerate(items):
                original_item = item_map.get(item, item)
                converted_recommendations.append({
                    'userno': original_user,
                    'Projectno': original_item
                })

        # 将推荐结果保存到CSV
        rec_df = pd.DataFrame(converted_recommendations)

        rec_df.to_csv(os.path.join(self.output_path, 'user_recommendations.csv'), index=False)
        return result
