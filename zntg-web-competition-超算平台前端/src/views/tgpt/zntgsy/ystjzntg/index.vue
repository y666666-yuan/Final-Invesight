<template>
  <div class="zntg">
    <div class="zntg-title">带约束条件的智能投顾实验</div>
    <div class="zntg-tab">
      <div
...
const getAlgorithmList = () => {
  // 获取已有算法
  ZntgsyAPI.getAlgorithmList({ algorithm_type: 2 }).then((data) => {
    if (data.length > 0) {
      data.map((item) => {
        if (item.constraint.length > 0) {
          item.constraint.map((ite) => {
            ite[ite.value] = [];
            ite.bol = false;
          });
        }
...
const postRunTask = () => {
  ...
  let intelligenceInfo = cloneDeep(state.intelligenceData[0]);
  let algorithmObj = {};
  let constraintParams = {};
  intelligenceInfo.property.map((item) => {
    algorithmObj[item.value] = item.default_value;
  });
  intelligenceInfo.constraint.map((item) => {
    if (item.bol) {
      constraintParams[item.value] = {
        category: item[item.value]
      }
    }
  });
