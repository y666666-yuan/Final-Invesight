...
import com.cjbdi.zhengqi.swufe.sypt.model.ForwardRequest;
...
public class GatewayFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        try {
            forwardRequest = getForwardRequest(requestUri);
        } catch (Exception e) {
        ...
        if (forwardRequest != null) {
            forwardRequest(httpRequest, httpResponse, forwardRequest);
        } else {
            chain.doFilter(request, response);
        }
    }

    private ForwardRequest getForwardRequest(String requestUri) throws Exception {
        for (ForwardingConfig.Route route : forwardingConfig.getRoutes()) {
            for (String path : route.getPaths()) {
                if (pathMatcher.match(path, requestUri)) {
                    ...
                    URL url = new URL(targetUrl + targetRequestUri);
                    ForwardRequest forwardRequest = new ForwardRequest();
                    forwardRequest.setTargetUrl(url);
                    forwardRequest.setRoute(route);
    ...
    private void forwardRequest(HttpServletRequest request,
        ...
        URL url = forwardRequest.getTargetUrl();
        Integer readTimeout = forwardRequest.getRoute().getReadTimeout();
        Integer connectionTimeout = forwardRequest.getRoute().getConnectionTimeout();
        try {
