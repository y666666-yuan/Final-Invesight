<template>
  <div class="zntg">
    <div class="zntg-title">导航式开发测试平台</div>
    <div class="zntg-tab">
      <div
    ...
    <div class="zntg-con" v-loading="zntgLoading" element-loading-text="结果计算中，请稍后">
      ...
      <Product
        ...
        @goNextStep="handleStep"
      />
      <Evaluate v-if="menuActive === 6" :taskId="taskId" :algorithmType="algorithmType" :algorithmName="algorithmName" @goNextStep="handleStep" />
    </div>
  </div>
</template>
...
const getAlgorithmList = () => {
  // 获取已有算法
  ZntgsyAPI.getAlgorithmList({ algorithm_type: 1 }).then((data) => {
    if (data.length > 0) {
      data.map((item) => {
        item.bol = false;
