    ...
    <div class="intelligence-main">
      ...
      <div class="intelligence-con">
        ...
        <!-- <div class="intelligence-line">
          <p></p>
          <p>算法参数</p>
          <p @click="closeSelect">一键恢复</p>
        </div>
        <div v-if="chackArr[intelligenceActive].property.length > 0" :class="['intelligence-checkbox', chackArr[intelligenceActive].name === '强化学习' ? 'checkbox-con_active':'']">
          <div
            v-for="(item, index) in chackArr[intelligenceActive].property"
            :key="index"
            :class="['checkbox-con', !isNotPlainObject(item[item.value]) ? 'checkbox-strengthen': '']"
          >
            <div class="custom-checkbox">
              <p class="custom-icon">*</p>
              <p>{{ item.label }}：</p>
            </div>
            <el-input v-if="isNotPlainObject(item[item.value])" v-model="item[item.value]" style="width: 480px" :placeholder="item.commit" />
            <div v-else class="strengthen-main">
              <div v-for="(ite, inde) in item['defaultArr']" :key="inde" class="strengthen-con">
                <el-input v-model="ite.name" style="width: 230px;margin-right: 20px;" :placeholder="item.commit" />
                <el-input v-model="ite.value" style="width: 230px" :placeholder="item.commit" />
                <p v-if="inde === 0" @click="handleStrengthen">添加</p>
              </div>
            </div>
          </div>
        </div> -->
